  ...
  <div class="container">
    <div class="form">
      <el-form size="mini" label-width="120px" :model="formData" :rules="formRule" ref="form">
        ...
        <el-form-item label="页面URL:" prop="pageUrl" v-if="!isButton" :required="!isButton">
...
export default {
    ...
    return {
      ...
      formRule: {
        ...
        pageUrl: [
          {
            validator: (rule, value, callback) => {
              if (MappingTools.ValueEqMapping(ResourceMapping.type.button, self.formData.type)) {
                callback()
              }
              if (!value) {
      ...
      treeProps: {
        ...
        label: 'name',
        children: 'menus',
        checkStrictly: true // 如果中间目录可以创建标签则加上这句话
      },
    };
  },
  async created () {
  ...
  methods: {
    ...
    async getTreeListFn () {
      const res = await AppResourceService.getTreeList({type: 1})
      if (res.code == 0) {
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          if (this.$route.query.id) {
            ...
            this.$alert(`新增资源成功，ID；${response.data}`)
          }
        }
      });
    }
